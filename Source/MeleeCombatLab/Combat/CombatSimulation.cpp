...
void Combatant::flinch(){state.flinch();if(state.phase==Phase::Idle){returnStart=local;returnAge=0;}}
bool Combatant::feint(const Tuning& t){if(!state.feint(t))return false;returnStart=local;returnAge=0;return true;}
bool Combatant::parry(const Tuning& t){if(!state.parry(t))return false;guard=view;returnStart=local;return true;}
void Combatant::look(double yawDelta,double pitchDelta,double dt,const Tuning& t)
{
    // No remainder is retained: discarded mouse motion cannot snap the view later.
...
void Combatant::reset(Vec at,Orientation facing,const Tuning& t)
{
    ...
    position=frameStart=frameTarget=at;view=desired=guard=simulatedView=frameViewStart=frameViewTarget=facing;health=100;damageTaken=0;hitsTaken=0;hitActors.clear();trackedSerial=0;
...
void Combatant::advance(double dt,const Tuning& t)
{
    ...
    returnAge+=dt;
    if(state.phase==Phase::Idle)local=blend(returnStart,AttackTrajectory::rest(),smooth(returnAge/.12));
    else if(state.phase==Phase::Parry)local=blend(returnStart,AttackTrajectory::evaluate(state,windupStart),smooth(state.elapsed/.055));
    else if(state.phase==Phase::Recovery&&oldPhase==Phase::Release){returnStart=before;local=before;}
    else if(state.phase==Phase::Recovery)local=blend(returnStart,AttackTrajectory::rest(),smooth(state.progress()));
    else local=AttackTrajectory::evaluate(state,windupStart);
...
{
    // Gather first. Resolve sorted contacts after detection, independent of actor tick/callback order.
    ...
    for(auto* a:actors){
        ...
        for(auto* d:actors){
            ...
            if(body&&a->state.damaging()&&a->state.releaseRotation<=tuning.AntiSpinThreshold*(a->state.isRiposte?tuning.RiposteTurnScale:1.))
                contacts.push_back({3,a,d,point,(point-a->weapon.hilt).length()});
        }
        if(worldSweep){for(auto sweep:a->traces){Vec point;++queriesLastFrame;if(worldSweep(a->id,sweep,tuning.BladeRadius,point)){
    ...
    for(auto c:contacts){auto& a=*c.a;auto* d=c.d;
        if(a.state.phase!=Phase::Release||(d&&a.hitActors.count(d->id)))continue;
        if(c.priority==0){if(!ChamberSystem::matches(d->state,ChamberSystem::inDefenderView(a.state.attack,a.view,d->view),tuning)||!d->state.spend(tuning.ChamberCost))continue;
            a.state.cancel(Resolution::Chamber);a.returnStart=a.local;d->state.last=Resolution::Chamber;emit(Resolution::Chamber,a,d,c.point);}
        else if(c.priority==1){if(d->state.phase!=Phase::Parry||!d->state.spend(tuning.ParryCost))continue;
            d->state.parrySuccess(tuning);a.state.cancel(Resolution::Parry);a.returnStart=a.local;emit(Resolution::Parry,a,d,c.point);}
        else if(c.priority==2){a.state.cancel(Resolution::Wall);a.returnStart=a.local;emit(Resolution::Wall,a,nullptr,c.point);}
        else if(d&&d->health>0){a.hitActors.insert(d->id);a.state.hitSomeone=true;d->damageTaken+=tuning.Damage;++d->hitsTaken;
            if(!d->infiniteHealth)d->health=std::max(0.,d->health-tuning.Damage);
            d->state.lastSpend=0;d->flinch();if(d->health==0)d->state.phase=Phase::Dead;emit(Resolution::Hit,a,d,c.point);}
    }
}
void CombatSimulation::advance(double dt)
{
    ...
    if(dt<=0)return;
    // Fixed 240Hz combat time; bounded catch-up retains unprocessed time rather than silently dropping it.
    constexpr double step=1./240.;accumulator+=dt;overload=accumulator>.25;
    for(auto* a:actors){a->frameStart=a->position;a->frameViewStart=a->simulatedView;a->frameViewTarget=a->view;}
    double processed=0;
    while(accumulator+1e-10>=step&&stepsLastFrame<64){
        const double alpha=clamp((processed+step)/dt,0.,1.);
        for(auto* a:actors){a->position=lerp(a->frameStart,a->frameTarget,alpha);
            if(a->externalView){a->view={wrap(a->frameViewStart.yaw+wrap(a->frameViewTarget.yaw-a->frameViewStart.yaw)*alpha),mix(a->frameViewStart.pitch,a->frameViewTarget.pitch,alpha)};a->desired=a->view;}}
