...
Latest combat motion and validation: [Kinetic swing revision](Docs/KINETIC_SWING_PASS.md).

Current presentation: [Citadel rebuild](../../Visual/CITADEL.md). Asset licenses and attribution: [Citadel credits](../../../ArtSource/Citadel/CREDITS.md).
...
If an open editor locks the default DLL, the script automatically builds a fresh numbered module and updates the launch manifest. Restart the editor after a successful build to load it; the existing session is not hot-reloaded or closed by the script.
...
The resumed [hands and hip-pivot lean pass](Docs/HANDS_AND_LEAN.md) adds connected gloves, shared camera/body/weapon/hurt-volume lean, world clearance and a 2 cm first-person near plane. Crouch half-height is 60 cm to keep the rigid torso above ground. [Review images and motion](../../Visual/Captures/HandsLean/overview.jpg) accompany the validation report.
